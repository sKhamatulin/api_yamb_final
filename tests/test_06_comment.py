...
class Test06CommentAPI:
    ...
    def test_02_comment(self, admin_client, admin):
        ...
        self.create_comment(admin_client, titles[0]["id"], reviews[0]["id"],
                            'qwerty')
        ...
        self.create_comment(admin_client, titles[0]["id"], reviews[1]["id"],
                            'qwerty432')
        self.create_comment(client_user, titles[0]["id"], reviews[1]["id"],
                            'qwerty534')
        response = self.create_comment(client_moderator, titles[0]["id"],
                                       reviews[1]["id"], 'qwerty231')
    ...
    def test_03_review_detail(self, client, admin_client, admin):
        ...
        data = {'text': 'rewq'}
        response = admin_client.patch(f'{pre_url}{comments[0]["id"]}/',
                                      data=data)
        assert response.status_code == 200, (
        ...
        client_user = auth_client(user)
        data = {'text': 'fgf'}
        response = client_user.patch(f'{pre_url}{comments[2]["id"]}/',
                                     data=data)
        ...
        data = {'text': 'jdfk'}
        response = client_user.patch(f'{pre_url}{comments[1]["id"]}/',
                                     data=data)
        assert response.status_code == 200, (
            'Проверьте, что при PATCH запросе `/api/v1/titles/{title_id}/reviews/{review_id}/comments/{comment_id}/` '
            'возвращается статус 200'
